...
*    2) on some ESP8266 such as the Wemos D1 mini, this is the same pin as serial comms!
* 
* Serial comms are disabled because of note (2) - you can uncomment debugging printf statements
* in this code below if your hardware supports the serial UART on a different pin to I2S.
*
...
#define LED_ON	0				// this is the value to write to GPIO for led on (GPIO Low)
#define LED_OFF 1				// this is the value to write to GPIO for led off (GPIO High)
#define LED_INBUILT_GPIO 2		// this is the onboard LED used to show on/off only
#define LED_STRIP_QUANTITY 16	// this is the number of WS2812B leds on the strip
#define LED_RGB_SCALE 255		// this is the scaling factor used for color conversion

// Global constants
const ws2812_pixel_t COLOR_WHITE = { { 255, 255, 255, 0 } };
const ws2812_pixel_t COLOR_RED = { { 255, 0, 0, 0 } };
const ws2812_pixel_t COLOR_GREEN = { { 0, 255, 0, 0 } };
...
float led_brightness = 100;	// brightness is scaped 0 to 100
bool led_on = false;		// on is boolean on or off
ws2812_pixel_t pixels[LED_STRIP_QUANTITY];
...
void ledStringFill(ws2812_pixel_t rgb) {

	// write out the new color each pixel
	for (int i = 0; i < LED_STRIP_QUANTITY; i++) {
		pixels[i] = rgb;
	}
	ws2812_i2s_update(pixels, PIXEL_RGB);
}

void ledStringSet(void) {

	ws2812_pixel_t rgb = COLOR_BLACK;

	if (led_on) {
		// convert HSI to RGBW
		hsi2rgb(led_hue, led_saturation, led_brightness, &rgb);

		// print debugging info - this won't work if serial comms are sharing GPIO3
		...
		// printf("off\n");
		gpio_write(LED_INBUILT_GPIO, LED_OFF);
	}
...
void led_init() {
	...
	ws2812_i2s_init(LED_STRIP_QUANTITY, PIXEL_RGB);
...
void led_identify_task(void *_args) {
	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			gpio_write(LED_INBUILT_GPIO, LED_ON);
			ledStringFill(COLOR_PINK);
			vTaskDelay(100 / portTICK_PERIOD_MS);
			gpio_write(LED_INBUILT_GPIO, LED_OFF);
			ledStringFill(COLOR_BLACK);
			vTaskDelay(100 / portTICK_PERIOD_MS);
		}
...
homekit_accessory_t *accessories[] = {
	HOMEKIT_ACCESSORY(.id = 1,.category = homekit_accessory_category_lightbulb,.services = (homekit_service_t*[]) {
		HOMEKIT_SERVICE(ACCESSORY_INFORMATION,.characteristics = (homekit_characteristic_t*[]) {
			&name,
			HOMEKIT_CHARACTERISTIC(MANUFACTURER, "Dave"),
			HOMEKIT_CHARACTERISTIC(SERIAL_NUMBER, "037A2BABF19D"),
			HOMEKIT_CHARACTERISTIC(MODEL, "MyLEDStrip"),
